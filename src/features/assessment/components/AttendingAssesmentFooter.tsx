...
const AttendingAssesmentFooter = ({
  ...
  return (
    <footer className="min-h-[50px] h-auto lg:h-[50px] bg-[#514CF1] text-white px-4 lg:px-8 flex flex-col lg:flex-row items-center justify-between fixed bottom-0 left-0 right-0 z-10 shadow-[0_-4px_20px_rgba(0,0,0,0.1)] py-4 lg:py-0 gap-4 lg:gap-0">
      {/* Left: Progress */}
      ...
      <div className="flex items-center justify-center lg:justify-end gap-2 lg:gap-4 w-full lg:w-auto flex-wrap order-3 lg:order-none">
        <button
          ...
          className="h-8 px-4 bg-white text-[#514CF1] rounded-[23px] text-xs font-bold hover:bg-white/90 transition-colors shadow-sm cursor-pointer flex-1 lg:flex-none"
        >
          Clear
        </button>
        <button
          onClick={actions.onMarkForReview}
          className="h-8 px-6 bg-[#FFFFFF] text-[#D97706] rounded-full text-sm font-bold hover:bg-[#fff7ed] transition-colors shadow-sm cursor-pointer flex-1 lg:flex-none"
        >
          Mark for Review
        </button>
        <button
          onClick={actions.onSaveNext}
          className="h-8 pl-6 pr-5 bg-white text-[#514CF1] rounded-full text-sm font-bold flex items-center justify-center gap-2 hover:bg-white/90 transition-colors shadow-sm cursor-pointer flex-1 lg:flex-none"
        >
          Save & Next
