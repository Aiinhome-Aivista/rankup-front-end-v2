...
import AttendingAssesmentHeader from "../components/AttendingAssesmentHeader";
import AttendingAssesmentFooter from "../components/AttendingAssesmentFooter";
        ...
        <div className="flex flex-col lg:flex-row gap-3 h-full items-stretch px-8">
          ...
          <aside className="w-full lg:w-[300px] bg-white rounded-[20px] px-6 py-3 shadow-sm border border-gray-100 lg:min-h-[527px] flex flex-col z-0">
            <div className="flex items-center gap-2 mb-8">
              ...
              <h2 className="text-[#514CF1] font-bold text-lg">
                Topics & Materials
              </h2>
            </div>
            ...
            <div className="grid grid-cols-2 gap-y-4 gap-x-2 mb-10">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-medium text-[#514CF1]">
                  Current
                </span>
              </div>
              <div className="flex items-center gap-2">
            ...
            <h3 className="text-[#514CF1] text-xs font-bold mb-4 uppercase tracking-wide">
              Section 1: Derivtives
            </h3>

            <div className="grid grid-cols-5 gap-3">
              {questions.map((q, idx) => {
                let bgClass =
                  "bg-[#4CAF501A] text-[#4CAF50] border border-[#4CAF5040]"; // Default style per snippet? No wait, snippet had varied.
                ...
                if (isCurrent) {
                  bgClass =
                    "bg-[#514CF1] text-white shadow-md shadow-[#514CF140]";
                } else if (q.status === "attempted") {
                  bgClass =
                    "bg-[#4CAF501A] text-[#4CAF50] border border-[#4CAF5040]";
          ...
          <div className="flex-1 rounded-[32px] px-2 lg:px-3  py-1 lg:py-3 lg:min-h-[500px]">
            <div className="flex flex-col sm:flex-row justify-between items-start mb-2 gap-4 sm:gap-0">
              <h2 className="text-[#514CF1] text-base font-bold">
                Question {currentQuestion.id}
              </h2>
              <div className="text-[#514CF1] text-sm font-bold">
                Points:{" "}
                <span className="bg-[#514CF10D] ml-2 px-3 py-2 rounded-2xl">
                  {currentQuestion.points.toFixed(1)}
                </span>
              </div>
            </div>

            <p
              className="text-[#1E293B] text-base font-normal mb-3 "
              dangerouslySetInnerHTML={{ __html: currentQuestion.text }}
            ></p>

            <p className="text-[#514CF1] text-sm font-medium mb-1">
              Select the closest answer from the options below.
            </p>

            <div className="border-2 border-dashed border-[#E2E8F0] rounded-3xl bg-[#F8FAFC] h-[137px] flex flex-col items-center justify-center gap-1 mb-4 cursor-pointer hover:bg-[#F1F5F9] transition-colors group">
              <div className="w-8 h-8 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform">
                <span className="material-symbols-outlined text-[#514CF1] text-2xl">
                  cloud_upload
                </span>
              </div>
              <div className="text-center">
                <p className="text-[#514CF1] text-sm mb-1">
            ...
            <div className="space-y-4 border-2 border-[#514CF10D] rounded-3xl p-4">
              <p className="text-[#514CF1] text-sm font-bold mb-4">
                Select your answer:
              </p>
              ...
              {currentQuestion.options.map((option, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`group flex px-2 items-center justify-between border rounded-2xl cursor-pointer transition-all hover:border-[#514CF1] hover:bg-[#514CF105] ${borderColor}`}
                  >
                    <div className="flex items-center gap-2 py-2.5">
                      <div
                        className={`w-4 h-4 rounded-full border-2 flex items-center justify-center bg-white group-hover:border-[#514CF1] ${dotBorder}`}
                      >
                        {isSelected && (
                          <div className="w-2 h-2 bg-[#514CF1] rounded-full" />
                        )}
                      </div>
                      <span className="text-[#514CF1] font-bold text-xs">
                        {option}
                      </span>
                    </div>
                    <div
                      className={`px-3 rounded-2xl border border-transparent bg-[#514CF10D] transition-colors ${
                        isSelected
                          ? "border-[#514CF1] text-[#514CF1]"
                          : "border-[#E2E8F0]"
                      }`}
                    >
                      <span
                        className={`text-xs font-bold group-hover:text-[#514CF1] ${
                          isSelected ? "text-[#514CF1]" : "text-[#94A3B8]"
                        }`}
                      >
